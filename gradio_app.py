...
def dynamicrafter_demo(result_dir='./tmp/', res=512):
    ...
    gpu_num = torch.cuda.device_count() if torch.cuda.is_available() else 0
    if gpu_num == 0:
        print("No GPUs found, running on CPU.")
        image2video = Image2Video(result_dir, resolution=resolution, gpu_num=0)
    else:
        print(f"Found {gpu_num} GPUs, using them for inference.")
        image2video = Image2Video(result_dir, resolution=resolution, gpu_num=gpu_num)
