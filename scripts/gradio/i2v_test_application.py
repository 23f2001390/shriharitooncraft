...
import torch.nn as nn # Import for DataParallel
...
class Image2Video():
    ...
    def get_image(self, image, prompt, steps=50, cfg_scale=7.5, eta=1.0, fs=3, seed=123, image2=None):
        ...
        if isinstance(model, nn.DataParallel):
            model_module = model.module # Access the underlying model
        else:
            model_module = model

        batch_size=1
        channels = model_module.model.diffusion_model.out_channels
        frames = model_module.temporal_length
        ...
        with torch.no_grad():
            ...
            with torch.cuda.amp.autocast() if self.device.type == 'cuda' else torch.no_grad():
                ...
                img_tensor2 = torch.from_numpy(image2).permute(2, 0, 1).float().to(self.device)
                ...
                z, hs = self.get_latent_z_with_hidden_states(model_module, videos)

                img_tensor_repeat = torch.zeros_like(z).to(self.device)

                img_tensor_repeat[:,:,:1,:,:] = z[:,:,:1,:,:]
                img_tensor_repeat[:,:,-1:,:,:] = z[:,:,-1:,:,:]


                cond_images = model_module.embedder(img_tensor.unsqueeze(0)).to(self.device) ## blc
                img_emb = model_module.image_proj_model(cond_images).to(self.device)
                ...
                ## inference
                batch_samples = batch_ddim_sampling(model_module, cond, noise_shape, n_samples=1, ddim_steps=steps, ddim_eta=eta, cfg_scale=cfg_scale, hs=hs)
        ...
        # No need to move model to CPU if DataParallel is used, or if it's already on CPU
